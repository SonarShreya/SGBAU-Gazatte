...
const Regulation = () => {
  const [regulationData, setRegulationData] = useState({
    ...
    dateOfPublication: "",
    concernSection: "",
    pdfFile: null, // Ensure file input is null initially
  });
  ...
  const searchHandle = async (event) => {
    ...
    if (key) {
      try {
        const response = await fetch(`http://localhost:5001/regulations/searchRegulations/${key}`);
        const result = await response.json();
        setRegulationInfo(result.length > 0 ? result : []);
      } catch (error) {
        console.error("Error searching Regulation info:", error);
      }
    } else {
  ...
  // Handle Add/Update Click
  const handleUpdateClick = (id) => {
    const regulationToUpdate = regulationInfo.find((info) => info._id === id);
    if (regulationToUpdate) {
      setRegulationData({
        _id: regulationToUpdate._id, // Store the ID to know it's an update
        srNo: regulationToUpdate.srNo,
        title: regulationToUpdate.title,
        draftRegulationNo: regulationToUpdate.draftRegulationNo,
        draftRegulationDate: regulationToUpdate.draftRegulationDate,
        ...
        dateOfPublication: regulationToUpdate.dateOfPublication,
        concernSection: regulationToUpdate.concernSection,
        pdfFile: regulationToUpdate.pdfFile, 
      });
      setShowForm(true);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("dateOfPublication", regulationData.dateOfPublication);
    formData.append("concernSection", regulationData.concernSection);
    if (regulationData.pdfFile instanceof File) {
    ...
    try {
      if (regulationData._id) {
        ...
        console.log(">>>>>>>>>>>>>>>>>>>>>>>>>")
        const response = await axios.patch(`http://localhost:5001/regulations/${regulationData._id}`, formData, {
      ...
      getRegulationInfo(); // Refresh data
    } catch (error) {
  ...
  return (
    <div className="table">
      <h1>Regulation Page</h1>
      <input type="text" placeholder="Search Regulation" value={searchQuery} onChange={searchHandle} />
      <button onClick={handleAddButtonClick}>Add Regulation</button>
      ...
      {!showForm ? (
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          <input type="text" name="title" value={regulationData.title} onChange={handleChange} required />
